...
const CategoriesPage = () => {
  ...
  const fetchCategories = async (page: number) => {
    try {
      ...
      const restaurantId = user.split("/")[2];
      ...
      if (role == "super-admin") {
        query.eq("restaurant_id", 0);
      } else {
        query.eq("restaurant_id", restaurantId).neq("restaurant_id", 0);
      }
    ...
    <section>
      ...
      {!isDataLoading && !restaurantId && role != "super-admin" ? (
        ...
        <div className="overflow-x-scroll">
          <table className="w-full table-auto rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark min-w-150">
