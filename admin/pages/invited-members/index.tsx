...
const InvitedMembersPage = () => {
  ...
  const fetchInvitedMembersData = async (page: number) => {
    try {
      ...
      console.error("Error while fetching invite members: ", error);
    }
  };
  ...
  const fetchCountInvitations = async () => {
    try {
      const user = await getCookiesValue("login-info");
      const { data, error } = await supabase
        .from("pending_invitations")
      ...
      console.error("Error while count invitations: ", error);
    }
  };
  ...
  const deleteRecord = async (id: number) => {
    try {
      ...
      console.error("Error while delete invitation: ", error);
    }
  };
    ...
    <section>
      ...
      {!isDataLoading && !restaurantId ? (
        ...
        <div className="overflow-x-scroll">
          <table className="w-full rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark min-w-180">
