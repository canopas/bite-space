...
import { logout, setSessionForUser } from "../../utils/jwt-auth";
...
const DropdownUser = React.memo(function Hello({ sessionUser }: any) {
  ...
  const trigger = useRef<any | null>(null);
  const dropdown = useRef<any | null>(null);
  const restro = useRef<any | null>(null);
  ...
  useEffect(() => {
    const clickHandler = ({ target }: MouseEvent) => {
      ...
      if (
        ...
        restro?.current?.contains(target) ||
  ...
  return (
    <div className="relative">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="h-12 w-12">
              <Image
                ...
                alt="restaurant-profile"
                className={`h-full w-full rounded-full object-cover ${
                  user.image ?? "border border-black"
                }`}
              />
            </span>
          </div>
      ...
      <div
        ref={dropdown}
        className={`absolute right-0 mt-4 flex max-h-96 w-64 flex-col overflow-y-scroll rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark ${
