...
const Admin = ({ paramsData }: { paramsData: { id: number } }) => {
  ...
  const [relativeId, setRelativeId] = useState<number>(0);
    ...
    const fetchAdmins = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .eq("admin_id", paramsData.id)
          .single();

        if (error) throw error;

        setRelativeId(data.id);
        setName(data.admins.name);
  ...
  const handleEditAdmin = async (e: any) => {
    ...
    try {
      ...
      const response = mySchema.safeParse({ name: name, email: email });
      ...
      let { error: relativeError } = await supabase
        .from("admins_roles_restaurants")
        .upsert({
          id: relativeId,
          admin_id: paramsData.id,
          role_id: roleId,
          restaurant_id: user.split("/")[2],
        });

      if (relativeError) throw relativeError;

      let { error } = await supabase.from("admins").upsert({
        id: paramsData.id,
        name: name,
        email: email,
        role: roleId,
