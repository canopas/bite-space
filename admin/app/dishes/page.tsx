  ...
  const fetchDishes = async (page: number) => {
    try {
      ...
      const { data: dishes, error } = await supabase
        .from("dishes")
        .select(`id, name, images, video, price, menus(name), categories(name)`)
        .range((page - 1) * pageSize, pageSize * page - 1)
        .eq("menus.restaurant_id", user.split("-")[2]);
    ...
    const fetchCountDishes = async () => {
      try {
        ...
        if (user.split("-")[2] != 0) {
          ...
          if (menus) setDishesCount(menus.length);
